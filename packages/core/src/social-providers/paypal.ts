...
import { APIError, ClearanceError } from "../error";
	...
	return {
		...
		async createAuthorizationURL({ state, codeVerifier, redirectURI }) {
			if (!options.clientId || !options.clientSecret) {
				...
				throw new ClearanceError("CLIENT_ID_AND_SECRET_REQUIRED");
			}
		...
		validateAuthorizationCode: async ({ code, redirectURI }) => {
			...
			try {
				...
				if (!response.data) {
					throw new ClearanceError("FAILED_TO_GET_ACCESS_TOKEN");
				}
		...
		refreshAccessToken: options.refreshAccessToken
			...
			: async (refreshToken) => {
					...
					try {
						...
						if (!response.data) {
							throw new ClearanceError("FAILED_TO_REFRESH_ACCESS_TOKEN");
						}
