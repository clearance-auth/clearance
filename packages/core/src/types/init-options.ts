...
import type { ClearancePlugin } from "./plugin";
...
export type DynamicBaseURLConfig = {
	 ...
	 * If not set, Clearance will throw an error when the host doesn't match.
	 *
...
export interface ClearanceRateLimitStorage {
...
export type ClearanceRateLimitRule = {
	/**
...
export type ClearanceDBOptions<
...
export type ClearanceRateLimitOptions = Optional<ClearanceRateLimitRule> &
	Omit<
		ClearanceDBOptions<"rateLimit", keyof BaseRateLimit>,
		"additionalFields"
	> & {
		/**
		...
		customRules?:
			| {
					[key: string]:
						...
						| ((
								request: Request,
								currentRule: ClearanceRateLimitRule,
						  ) => Awaitable<false | ClearanceRateLimitRule>);
			  }
			| undefined;
		/**
		 ...
		 * is ignored
		 */
		customStorage?: ClearanceRateLimitStorage;
	};

export type ClearanceAdvancedOptions = {
	/**
	...
	ipAddress?:
		| {
				 ...
				 *
				 * @default
				 * @link https://github.com/clearance-auth/clearance
				 */
	 ...
	 * @default
	 * ```txt
	 * "appName" -> which defaults to "clearance"
	 * ```
	 */
...
export type ClearanceOptions = {
	...
	/**
	 * Base URL for the Clearance. This is typically the
	 ...
	 * `CLEARANCE_URL`, `NEXT_PUBLIC_CLEARANCE_URL`, etc.
	 *
	 * @example
	 * ```ts
	 * // Static URL
	...
	/**
	 * Base path for the Clearance. This is typically
	 * the path where the
	 * Clearance routes are mounted.
	 *
	 * @default "/api/auth"
	 */
	 ...
	 * signing and hashing.
	 *
	 * By default Clearance will look for
	 ...
	 *
	 * Can also be set via CLEARANCE_SECRETS env var:
	 * `CLEARANCE_SECRETS=2:base64secret,1:base64secret`
	 *
	...
	/**
	 * List of Clearance plugins
	 */
	plugins?: ([] | ClearancePlugin[]) | undefined;
	/**
	 * User configuration
	 */
	user?:
		| (ClearanceDBOptions<"user", keyof BaseUser> & {
				/**
		...
		| undefined;
	session?:
		| (ClearanceDBOptions<"session", keyof BaseSession> & {
				/**
		...
		| undefined;
	account?:
		| (ClearanceDBOptions<"account", keyof BaseAccount> & {
				/**
				 ...
				 * still hit browser or proxy cookie/header limits even though Clearance
		...
		| undefined;
	verification?:
		| (ClearanceDBOptions<"verification", keyof BaseVerification> & {
				/**
		...
		| undefined;
	/**
	 * Additional trusted origins. By default, Clearance trusts your
	 ...
	 *   return [
	 *    "https://github.com/clearance-auth/clearance",
	 *    "https://*.clearance.com",
	 ...
	 */
	rateLimit?: ClearanceRateLimitOptions | undefined;
	/**
	 * Advanced options
	 */
	advanced?: ClearanceAdvancedOptions | undefined;
	...
	onAPIError?:
		| {
				...
				errorURL?: string;
				/**
				 * Configure the default error page provided by Clearance
