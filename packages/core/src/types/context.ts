import type { CookieOptions, EndpointContext } from "@clearance/call";
import type {
	Account,
	ClearanceDBSchema,
	ModelNames,
 ...
 * @internal
 */
type InferPluginID<O extends ClearanceOptions> =
	O["plugins"] extends Array<infer P>
		? P extends ClearancePlugin
			? P["id"]
			: never
		: never;
...
type InferPluginOptions<
	O extends ClearanceOptions,
	ID extends ClearancePluginRegistryIdentifier | LiteralString,
> =
	O["plugins"] extends Array<infer P>
		? P extends ClearancePlugin
			? P["id"] extends ID
...
export interface ClearancePluginRegistry<AuthOptions, Options> {}
export type ClearancePluginRegistryIdentifier = keyof ClearancePluginRegistry<
	unknown,
	unknown
>;
...
export interface InternalAdapter<
	_Options extends ClearanceOptions = ClearanceOptions,
> {
	createOAuthUser(
...
type CheckPasswordFn<Options extends ClearanceOptions = ClearanceOptions> = (
	userId: string,
	ctx: GenericEndpointContext<Options>,
) => Promise<boolean>;

export type PluginContext<Options extends ClearanceOptions> = {
	getPlugin: <
		ID extends ClearancePluginRegistryIdentifier | LiteralString,
	...
	>(
		...
		| (ID extends ClearancePluginRegistryIdentifier
				? ClearancePluginRegistry<Options, PluginOptions>[ID] extends {
						creator: infer C;
					}
	 ...
	 * ```
	 */
	hasPlugin: <ID extends ClearancePluginRegistryIdentifier | LiteralString>(
		pluginId: ID,
...
export type AuthContext<Options extends ClearanceOptions = ClearanceOptions> =
