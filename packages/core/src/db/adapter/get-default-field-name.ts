...
export const initGetDefaultFieldName = ({
	...
	const getDefaultFieldName = ({
		...
		// Plugin `schema`s can't define their own `id`. Clearance auto provides `id` to every schema model.
		...
		if (!f) {
			throw new ClearanceError(`Field ${field} not found in model ${model}`);
		}
		return field;
	};
