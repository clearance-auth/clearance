...
import { ClearanceError } from "../../error";
...
export type AdapterFactory<Options extends ClearanceOptions> = (
	options: Options,
) => DBAdapter<Options>;

export const createAdapterFactory =
	<Options extends ClearanceOptions>({
	...
	(options: Options): DBAdapter<Options> => {
		...
		// End-user's Clearance instance's schema
			...
			return where.map((w) => {
				...
				if (operator === "in") {
					if (!Array.isArray(value)) {
						throw new ClearanceError("Value must be an array");
					}
				}
		...
		const adapter: DBAdapter<Options> = {
			...
			consumeOne: async <T>({
				...
				if (adapterInstance.consumeOne) {
					...
					// Use Clearance's transaction context here, not a direct adapter
