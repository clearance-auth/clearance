...
import { refreshAccessToken } from "@clearance/core/oauth2";
import type {
	GoogleProfile,
...
import { reddit } from "@clearance/core/social-providers";
	...
	it("should be able to sign in with social providers", async () => {
		...
		await client.$fetch("/callback/google", {
			...
			onError(context) {
				...
				expect(cookies.get("clearance.session_token")?.value).toBeDefined();
			},
		});
	});
		...
		const signInRes = await client.signIn.social(
			...
			},
			{
				onSuccess(context) {
					...
					headers.set(
						"cookie",
						`clearance.state=${cookies.get("clearance.state")?.value}`,
					);
				},
			},
		);
...
describe("Apple Provider", async () => {
	...
	it("should use firstName and lastName when provided in token.user", async () => {
		...
		expect(session.data?.user.name).toBe("Clearance");
	});
...
describe("Railway Provider", async () => {
		...
		mswServer.use(
			http.post(
				...
				async ({ request }) => {
					...
					// Verify PKCE code_verifier is present (Clearance uses PKCE by default)
					const codeVerifier = params.get("code_verifier");
					expect(codeVerifier).not.toBeNull();
					expect(codeVerifier).not.toBe("");
	...
	it("should initiate Railway OAuth flow with PKCE", async () => {
		...
		// Verify PKCE parameters are present (Clearance uses PKCE by default)
		expect(authUrl.searchParams.get("code_challenge")).not.toBeNull();
		expect(authUrl.searchParams.get("code_challenge_method")).toBe("S256");
	});
