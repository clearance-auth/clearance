...
export type InferClientAPI<O extends ClearanceClientOptions> = InferRoutes<
...
export type InferActions<O extends ClearanceClientOptions> =
...
export type InferErrorCodes<O extends ClearanceClientOptions> =
	O["plugins"] extends Array<infer Plugin>
		? UnionToIntersection<
				Plugin extends {
...
export type InferSessionFromClient<O extends ClearanceClientOptions> =
	StripEmptyObjects<
		Session &
			UnionToIntersection<InferAdditionalFromClient<O, "session", "output">>
	>;
export type InferUserFromClient<O extends ClearanceClientOptions> =
	StripEmptyObjects<
		User & UnionToIntersection<InferAdditionalFromClient<O, "user", "output">>
	>;

export type InferAdditionalFromClient<
	Options extends ClearanceClientOptions,
	Key extends string,
