...
describe("client declaration emit", () => {
	const createCompositeProject = (): { dir: string; cleanup: () => void } => {
		const dir = fs.mkdtempSync(path.join(os.tmpdir(), "clearance-client-dts-"));
		const clearanceDir = path.resolve(__dirname, "../..");
		...
		fs.mkdirSync(path.join(authPackageDir, "node_modules/@clearance"), {
			recursive: true,
		});
		...
		fs.symlinkSync(
			clearanceDir,
			path.join(authPackageDir, "node_modules/@clearance/runtime"),
			"junction",
		);
		fs.symlinkSync(
			authPackageDir,
		...
		fs.writeFileSync(
			...
			`
import { createAuthClient as createClearanceClient } from "@clearance/runtime/react";
