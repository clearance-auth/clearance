	...
	const dbUser = await c.context.internalAdapter
		...
		.catch((e) => {
			c.context.logger.error(
				"Clearance was unable to query your database.\nError: ",
				e,
			);
			const errorURL =
			...
			if (
				...
				if (isDevelopment()) {
					c.context.logger.warn(
						`User already exists but the account is not linked to ${account.providerId}. See https://github.com/clearance-auth/clearance`,
					);
				}
				return {
