...
import { serializeCookie } from "@clearance/call";
...
describe("cookie configuration", () => {
	it("should return correct cookie options based on configuration", async () => {
		const options = {
			baseURL: "https://example.com",
			database: {} as ClearanceOptions["database"],
			advanced: {
...
describe("cookie-utils stripSecureCookiePrefix", () => {
	...
	it("should handle cookie names with dots and special characters", () => {
		const cookieName = `${SECURE_COOKIE_PREFIX}clearance.session_token`;
		const result = stripSecureCookiePrefix(cookieName);
		expect(result).toBe("clearance.session_token");
	});
});
...
describe("cookie-utils setRequestCookie", () => {
	it("writes a cookie when the header is empty", () => {
		...
		setRequestCookie(headers, "clearance.session_token", "abc");
		expect(headers.get("cookie")).toBe("clearance.session_token=abc");
	});
	...
	it("preserves existing cookies and joins with `; ` per RFC 6265", () => {
		...
		expect(headers.get("cookie")).toBe(
			"preference=dark; locale=en; clearance.session_token=abc",
		);
	});

	it("replaces an existing cookie of the same name rather than duplicating it", () => {
		const headers = new Headers({
			cookie: "clearance.session_token=stale; locale=en",
		});
		setRequestCookie(headers, "clearance.session_token", "fresh");
		expect(headers.get("cookie")).toBe(
			"clearance.session_token=fresh; locale=en",
		);
	});
	...
	it("ignores malformed pairs in the existing header", () => {
		...
		setRequestCookie(headers, "clearance.session_token", "abc");
		expect(headers.get("cookie")).toBe(
			"valid=1; locale=en; clearance.session_token=abc",
		);
	});
...
describe("getSessionCookie", async () => {
	...
	describe.each([".", "-"])("with '%s' separator", (separator) => {
		describe.each([
			...
			it.each([
				...
				[{ cookieName: "my_token" }, "clearance", "my_token"],
				[
	...
	it("prefers the __Secure- cookie when a non-secure leftover is also present", () => {
		...
		headers.set(
			"cookie",
			"clearance.session_token=stale; __Secure-clearance.session_token=current",
		);
		const request = new Request("https://example.com/api/auth/session", {
			headers,
		});
	...
	it("does not fall back to a non-secure cookie when the __Secure- value is empty", () => {
		...
		headers.set(
			"cookie",
			"clearance.session_token=stale; __Secure-clearance.session_token=",
		);
		const request = new Request("https://example.com/api/auth/session", {
			headers,
		});
		...
		await client.signIn.email(
			...
			},
			{
				onSuccess(c) {
					...
					const sessionTokenCookie = parsed.get("clearance.session_token")!;
					expect(sessionTokenCookie).toBeDefined();
					expect(sessionTokenCookie["max-age"]).toBeUndefined();

					const sessionDataCookie = parsed.get("clearance.session_data")!;
...
describe("Cookie Cache Field Filtering", () => {
	...
	it("should return null for invalid JWT token", async () => {
		...
		headers.set("cookie", "clearance.session_data=invalid.jwt.token");
...
describe("Cookie Chunking", () => {
	...
	it("should chunk session cache when attributes push the line over the limit", async () => {
		...
		const longPrefix = "clearance-" + "x".repeat(80);
		const { client } = await getTestInstance({
			secret: "clearance.secret",
	...
	it("skips the session cache and warns when the prefix alone overflows", async () => {
		...
		const longPrefix = "clearance-" + "x".repeat(4100);
		const { client, auth, cookieSetter } = await getTestInstance({
			secret: "clearance.secret",
			advanced: { cookiePrefix: longPrefix },
			logger: {
...
describe("parse cookies", () => {
	it("should parse cookies into key-value map", () => {
		const cookieHeader =
			"clearance.session_token=session-token.signature; clearance.session_data=session-data.signature";

		const parsedCookies = parseCookies(cookieHeader);

		expect(parsedCookies.get("clearance.session_token")).toBe(
			"session-token.signature",
		);
		expect(parsedCookies.get("clearance.session_data")).toBe(
...
describe("Cookie header without whitespace after semicolon", () => {
	it("parseCookies returns each pair when separator is `;` only", () => {
		...
		expect(parsed.get("clearance.session_token")).toBe(
			"session-token.signature",
		);
		expect(parsed.get("clearance.session_data")).toBe(
			"session-data.signature",
		);
	});
	...
	it("getSessionCookie finds the session cookie when separator is `;` only", () => {
		...
		headers.set(
			"cookie",
			"preference=dark;clearance.session_token=token-123",
		);
		const request = new Request("https://example.com/api/auth/session", {
			headers,
		});
	...
	it("getChunkedCookie reconstructs chunks across `;`-only separators", () => {
		...
		headers.set(
			"cookie",
			"clearance.session_data.0=chunkA;clearance.session_data.1=chunkB",
		);
		const ctx = {
			getCookie: () => undefined,
			headers,
		...
		expect(getChunkedCookie(ctx, "clearance.session_data")).toBe(
			"chunkAchunkB",
		);
	});
});
...
describe("getCookieCache expiry (compact strategy)", () => {
	...
	function requestWith(cookieValue: string) {
		const headers = new Headers();
		headers.set("cookie", `clearance.session_data=${cookieValue}`);
