...
export function createCookieGetter(options: ClearanceOptions) {
	const baseURLString =
	...
	function createCookie(
		...
		const prefix = options.advanced?.cookiePrefix || "clearance";
		const name =
		...
		} satisfies ClearanceCookie;
	}
	return createCookie;
}

export function getCookies(options: ClearanceOptions) {
...
export type EligibleCookies = (string & {}) | (keyof ClearanceCookies & {});
...
export const getSessionCookie = (
	...
	const { cookieName = "session_token", cookiePrefix = "clearance" } =
		config || {};
...
>(
	...
	const { cookieName = "session_data", cookiePrefix = "clearance" } =
		config || {};
	const name =
	...
	if (sessionData) {
		const secret = config?.secret || env.CLEARANCE_SECRET;
		if (!secret) {
			throw new ClearanceError(
				"getCookieCache requires a secret to be provided. Either pass it as an option or set the CLEARANCE_SECRET environment variable",
			);
		}
