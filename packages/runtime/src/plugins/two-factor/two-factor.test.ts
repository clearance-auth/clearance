...
describe("two factor", async () => {
	...
	it("should fallback to appName when no issuer provided", async () => {
		...
		const totpURI = res.data?.totpURI;
		expect(totpURI).toMatch(/^otpauth:\/\/totp\/Clearance:/);
		expect(totpURI).toContain("&issuer=Clearance&");
	});
		...
		const res = await client.signIn.email({
			...
			fetchOptions: {
				onResponse(context) {
					...
					expect(parsed.get("clearance.session_token")?.value).toBe("");
					expect(parsed.get("clearance.session_data")?.value).toBe("");
					expect(parsed.get("clearance.two_factor")?.value).toBeDefined();
					...
					headers.append(
						"cookie",
						`clearance.dont_remember=${
							parsed.get("clearance.dont_remember")?.value
						}`,
					);
				},
			},
		...
		const verifyRes = await client.twoFactor.verifyOtp({
			...
			fetchOptions: {
				...
				onResponse(context) {
					...
					expect(parsed.get("clearance.session_token")?.value).toBeDefined();
					// max age should be undefined because we are not using remember me
					expect(
						parsed.get("clearance.session_token")?.["max-age"],
					).not.toBeDefined();
				},
			},
		});
		...
		const verifyRes = await client.twoFactor.verifyOtp({
			...
			fetchOptions: {
				...
				onResponse(context) {
					...
					expect(
						parsed.get("clearance.session_token")?.value,
					).not.toBeDefined();
				},
			},
		});
	...
	it("should allow sign in with backup code", async () => {
		...
		await client.signIn.email({
			...
			fetchOptions: {
				onSuccess(context) {
					...
					const token = parsed.get("clearance.session_token")?.value;
					expect(token).toBe("");
					headers.append(
						"cookie",
						`clearance.two_factor=${
		...
		const token = parsedCookies.get("clearance.session_token")?.value;
		...
		await client.twoFactor.sendOtp({
			fetchOptions: {
				...
				onSuccess(context) {
					...
					headers.append(
						"cookie",
						`clearance.otp.counter=${
							parsed.get("clearance.otp_counter")?.value
						}`,
					);
				},
			},
		...
		await client.twoFactor.verifyOtp({
			...
			fetchOptions: {
				...
				onSuccess(context) {
					...
					expect(parsed.get("clearance.trust_device")?.value).toBeDefined();
					newHeaders.set(
						"cookie",
						`clearance.trust_device=${
							parsed.get("clearance.trust_device")?.value
						}`,
					);
				},
			},
...
describe("two factor auth API", async () => {
	...
	it("should request second factor", async () => {
		...
		const twoFactorCookie = parsed.get("clearance.two_factor");
		expect(twoFactorCookie).toBeDefined();
		const sessionToken = parsed.get("clearance.session_token");
		...
		await client.twoFactor.verifyBackupCode({
			...
			fetchOptions: {
				...
				onSuccess(context) {
					...
					sessionToken = parsed.get("clearance.session_token")?.value || "";
				},
			},
		});
...
describe("trust device server-side validation", async () => {
	...
	it("should force 2FA when server-side trust record is expired", async () => {
		...
		trustHeaders.set(
			"cookie",
			`clearance.trust_device=${trustDeviceCookieValue}`,
		);
		...
		const clearedTrustCookie = signIn2Parsed.get("clearance.trust_device");
	...
	it("should revoke trust device when disabling 2FA", async () => {
		...
		const clearedCookie = disableParsed.get("clearance.trust_device");
...
describe("trustDeviceMaxAge", async () => {
	...
	it("should use custom trustDeviceMaxAge for the trust device cookie", async () => {
		...
		const trustDeviceCookie = parsed.get("clearance.trust_device");
