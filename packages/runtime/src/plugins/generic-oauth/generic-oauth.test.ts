...
describe("oauth2", async () => {
	...
	it("should delete state cookie with path attribute", async () => {
		...
		const stateCookie = cookies.get("clearance.state");
			...
			},
			{
				onSuccess(context) {
					...
					headers.set(
						"cookie",
						`clearance.state=${
							parsedSetCookie.get("clearance.state")?.value
						}; clearance.pk_code_verifier=${
							parsedSetCookie.get("clearance.pk_code_verifier")?.value
						}`,
					);
				},
			},
