...
describe("email-otp verify-email cookie cache isolation", async () => {
	const secret = "clearance-secret-1234567890-cookie-cache";
	let otp = "";
	const { client, auth } = await getTestInstance(
		{
			secret,
	...
	it("should not mark the current session's user as verified when a different user's email is verified", async () => {
		...
		await client.signIn.email(
			{ email: currentUserEmail, password: currentUserPassword },
			{
				onSuccess(ctx) {
					...
					const token = cookies.get("clearance.session_token")?.value;
					const data = cookies.get("clearance.session_data")?.value;
					currentUserHeaders.set(
						"cookie",
						`clearance.session_token=${token}; clearance.session_data=${data}`,
					);
				},
			},
		);
		...
		const verified = await client.emailOtp.verifyEmail(
			{ email: otherEmail, otp },
			{
				...
				onSuccess(ctx) {
					...
					refreshedSessionData = cookies.get("clearance.session_data")?.value;
				},
			},
		);
		...
		if (refreshedSessionData) {
			...
			currentUserHeaders.set(
				"cookie",
				`clearance.session_token=${token}; clearance.session_data=${refreshedSessionData}`,
			);
		}
