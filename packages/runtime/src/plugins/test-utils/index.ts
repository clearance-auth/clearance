...
/**
 * Test utilities plugin for Clearance.
 *
 ...
 * import { testUtils } from "@clearance/runtime/plugins";
 *
 * export const testAuth = clearance({
 *   plugins: [
		...
		init(ctx) {
			...
			const databaseHooks = options.captureOTP
				? ({
					...
					} satisfies ClearanceOptions["databaseHooks"])
				: null;
