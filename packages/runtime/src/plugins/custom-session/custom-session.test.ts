		...
		await client.getSession({
			fetchOptions: {
				...
				onResponse(context) {
					...
					expect(joined).toContain("clearance.session_token");
					expect(joined).toContain("clearance.session_data");
				},
			},
		});
	});
			...
			fetchOptions: {
				...
				onResponse(context) {
					...
					const tokenCookie = setCookies.find((c) =>
						c.includes("clearance.session_token"),
					);
					const dataCookie = setCookies.find((c) =>
						c.includes("clearance.session_data"),
					);
			...
			fetchOptions: {
				...
				onResponse(context) {
					...
					for (const cookie of setCookies) {
						...
						// None of those attributes should be another "clearance." cookie
						const clearanceEntries = cookieNames.filter((n) =>
							n.startsWith("clearance."),
						);
						expect(clearanceEntries).toHaveLength(1);
					}
				},
			},
		});
		...
		await partitionedClient.getSession({
			fetchOptions: {
				...
				onResponse(context) {
					...
					const clearanceCookies = parsedCookies.filter(([name]) =>
						name.startsWith("clearance."),
					);

					expect(clearanceCookies.length).toBeGreaterThan(0);
					for (const [, attributes] of clearanceCookies) {
