...
export const magicLink = (options: MagicLinkOptions) => {
	...
	if (options.allowedAttempts !== undefined && options.allowedAttempts !== 1) {
		console.warn(
			"[clearance/magic-link] `allowedAttempts` is ignored: tokens are consumed atomically on the first verification call. Any value other than `1` has no effect; remove the option to silence this warning.",
		);
	}
