...
export interface LastLoginMethodOptions {
	 ...
	 * @default "clearance.last_used_login_method"
	 */
...
export const lastLoginMethod = <O extends LastLoginMethodOptions>(
	...
	const config = {
		cookieName: "clearance.last_used_login_method",
		maxAge: 60 * 60 * 24 * 30,
		...userConfig,
			...
			after: [
				{
					...
					handler: createAuthMiddleware(async (ctx) => {
						...
						if (lastUsedLoginMethod) {
							...
							if (hasSessionToken) {
								// Inherit cookie attributes from Clearance's centralized cookie system
