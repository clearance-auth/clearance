...
describe("lastLoginMethod", async () => {
	...
	it("should set the last login method cookie", async () => {
		...
		expect(cookies.get("clearance.last_used_login_method")).toBe("email");
	});
	...
	it("should set the last login method cookie for siwe", async () => {
		...
		const cookies = parseCookies(headers.get("cookie") || "");
		expect(cookies.get("clearance.last_used_login_method")).toBe("siwe");
	});
		...
		await client.$fetch("/magic-link/verify", {
			...
			onError(context) {
				...
				const lastMethod = cookies.get(
					"clearance.last_used_login_method",
				)?.value;
			...
			},
			{
				...
				onError(context) {
					...
					if (context.response.status === 302) {
						...
						const sessionCookie = cookies.get("clearance.session_token");
	...
	it("should NOT set the last login method cookie on failed authentication", async () => {
		...
		const cookies = parseCookies(headers.get("cookie") || "");
		expect(cookies.get("clearance.last_used_login_method")).toBeUndefined();
	});
	...
	describe("beforeStoreCookie hook", () => {
		...
		it("should conditionally set cookie based on login method", async () => {
			...
			expect(emailCookies.get("clearance.last_used_login_method")).toBe(
				"email",
			);
			...
			expect(
				oauthCookies.get("clearance.last_used_login_method"),
			).toBeUndefined();
		});
		...
		it("should conditionally set cookie based on context properties", async () => {
			...
			expect(signInCookies.get("clearance.last_used_login_method")).toBe(
				"email",
			);
		});
		...
		it("should allow different behavior for different login methods", async () => {
			...
			const oauthCookies = parseCookies(oauthHeaders.get("cookie") || "");
			expect(oauthCookies.get("clearance.last_used_login_method")).toBe(
				"google",
			);
		});
		...
		it("should handle dynamic consent changes between logins", async () => {
			...
			expect(
				cookies1.get("clearance.last_used_login_method"),
			).toBeUndefined();
			...
			expect(cookies2.get("clearance.last_used_login_method")).toBe("email");
		});
		...
		it("should NOT set cookie when beforeStoreCookie throws an error", async () => {
			...
			if (responseHeaders) {
				...
				expect(
					setCookie.get("clearance.last_used_login_method"),
				).toBeUndefined();
			}
		});
		...
		it("should handle complex GDPR consent logic", async () => {
			...
			const cookies1 = parseCookies(headers1.get("cookie") || "");
			expect(cookies1.get("clearance.last_used_login_method")).toBe("email");
		});
	...
	it("should clear cross-subdomain cookies by including the domain attribute", async () => {
		...
		expect(setCookieHeader).toContain(
			"clearance.last_used_login_method=email",
		);
		...
		Object.defineProperty(globalThis.document, "cookie", {
			get() {
				return "clearance.last_used_login_method=email";
			},
			set(val: string) {
				writtenCookie = val;
			},
