...
import type { Prettify } from "@clearance/call";
...
type InferSchema<
	Schema extends ClearancePluginDBSchema,
	...
	(O["dynamicAccessControl"] extends { enabled: true }
		? {
				organizationRole: InferSchema<
					O["schema"] extends ClearancePluginDBSchema ? O["schema"] : {},
					"organizationRole",
		...
		(O["teams"] extends { enabled: true }
			...
			>;
			invitation: {
				modelName: O["schema"] extends ClearancePluginDBSchema
					? InferSchema<
							O["schema"],
							"invitation",
