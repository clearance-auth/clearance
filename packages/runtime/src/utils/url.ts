import type { BaseURLConfig, DynamicBaseURLConfig } from "@clearance/core";
 ...
 * `@clearance/core/utils/host` classifier here: its `utils/ip` dependency
 ...
 * `isLoopbackHost` from `@clearance/core/utils/host`. This helper's only
...
export function getBaseURL(
	...
	if (loadEnv !== false) {
		const fromEnv =
			env.CLEARANCE_URL ||
			env.NEXT_PUBLIC_CLEARANCE_URL ||
			env.PUBLIC_CLEARANCE_URL ||
			env.NUXT_PUBLIC_CLEARANCE_URL ||
 ...
 * @throws ClearanceError if host is not in allowedHosts and no fallback is set
 */
