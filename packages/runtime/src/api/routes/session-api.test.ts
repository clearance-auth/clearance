		...
		await client.signIn.email(
			...
			},
			{
				onSuccess(context) {
					...
					const cookie = cookies.get("clearance.session_token");
			...
			const response = await client.getSession({
				fetchOptions: {
					...
					onSuccess(context) {
						...
						const maxAge = parsed.get("clearance.session_token")?.["max-age"];
		...
		await client.signIn.email(
			...
			},
			{
				onSuccess(context) {
					const header = context.response.headers.get("set-cookie");
					const cookies = parseSetCookieHeader(header || "");
					expect(cookies.get("clearance.session_token")).toMatchObject({
						value: expect.any(String),
						"max-age": 60 * 60 * 24 * 7,
						path: "/",
						httponly: true,
...
describe("cookie cache with JWT strategy", async () => {
	...
	it("should not allow tampering with the cookie", async () => {
		...
		headers.set("cookie", `clearance.session_data=${newJWT}`);
		headers.append("cookie", `clearance.session_token=${sessionCookie}`);
		...
		await client.getSession({
			fetchOptions: {
				...
				onSuccess(context) {
					...
					sessionTokenMaxAge = parsed.get("clearance.session_token")?.[
						"max-age"
					];
				},
			},
