...
export const signInSocial = <O extends ClearanceOptions>() =>
	createAuthEndpoint(
		"/sign-in/social",
		{
			method: "POST",
...
export const signInEmail = <O extends ClearanceOptions>() =>
	createAuthEndpoint(
		"/sign-in/email",
		{
			method: "POST",
		...
		async (
			...
			if (!ctx.context.options?.emailAndPassword?.enabled) {
				ctx.context.logger.error(
					"Email and password is not enabled. Make sure to enable it in the options in your `auth.ts` file. See `https://github.com/clearance-auth/clearance` for more.",
				);
