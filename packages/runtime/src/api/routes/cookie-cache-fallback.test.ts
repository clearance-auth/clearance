 ...
 * When HMAC verification fails on the old `session_data` cookie, Clearance
...
describe("cookieCache HMAC verification failure fallback", async () => {
	...
	it("should fall through to session_token DB validation when session_data HMAC fails", async () => {
		...
		const sessionToken = cookies.get("clearance.session_token");
		...
		tampered.set(
			"cookie",
			`clearance.session_data=${invalidSessionData}; clearance.session_token=${sessionToken}`,
		);
	...
	it("should still return null when both session_data and session_token are invalid", async () => {
		...
		tampered.set(
			"cookie",
			`clearance.session_data=${invalidSessionData}; clearance.session_token=invalid-token.invalid-signature`,
		);
	...
	it("should work with JWT strategy when HMAC verification fails", async () => {
		...
		tampered.set(
			"cookie",
			`clearance.session_data=invalid.jwt.token; clearance.session_token=${sessionToken}`,
		);
	...
	it("should work with JWE strategy when decryption fails", async () => {
		...
		tampered.set(
			"cookie",
			`clearance.session_data=invalid.jwe.token.here.test; clearance.session_token=${sessionToken}`,
		);
	...
	it("should handle cross-subdomain cookie migration without silent logout", async () => {
		...
		const cookies = parseCookies(cookieStr);
		const sessionToken =
			cookies.get("__Secure-clearance.session_token") ||
			cookies.get("clearance.session_token");
		expect(sessionToken).toBeDefined();
		...
		migrationHeaders.set(
			"cookie",
			`__Secure-clearance.session_data=${staleSessionData}; __Secure-clearance.session_token=${sessionToken}`,
		);
	...
	it("should work during cookieCache window even with stale session_data", async () => {
		...
		migrationHeaders.set(
			"cookie",
			`clearance.session_data=${staleSessionData}; clearance.session_token=${sessionToken}`,
		);
