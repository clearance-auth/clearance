...
export async function runPluginInit(context: AuthContext) {
	...
	for (const plugin of plugins) {
		if (plugin.init) {
			...
			let result: ReturnType<Required<ClearancePlugin>["init"]>;
...
export function getInternalPlugins(options: ClearanceOptions) {
	const plugins: ClearancePlugin[] = [];
...
export async function getTrustedOrigins(
	...
	const envTrustedOrigins = env.CLEARANCE_TRUSTED_ORIGINS;
 ...
 * and cookies rehydrated for the resolved host. Throws `ClearanceError`
...
export async function resolveRequestContext(
	...
	const trustedOriginOptions: ClearanceOptions = {
		...resolved.options,
