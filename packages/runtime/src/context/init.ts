...
import { getKyselyDatabaseType } from "@clearance/kysely-adapter";
...
export const init = async (options: ClearanceOptions) => {
	const adapter = await getAdapter(options);

	// Get database type using Kysely's dialect detection
	const getDatabaseType = (database: ClearanceOptions["database"]) =>
