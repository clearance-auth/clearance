...
import type { createLogger } from "@clearance/core/env";
	...
	const entries = envValue.split(",").map((entry) => {
		...
		if (colonIdx === -1) {
			throw new ClearanceError(
				`Invalid CLEARANCE_SECRETS entry: "${entry}". Expected format: "<version>:<secret>"`,
			);
		}
		const version = parseInt(entry.slice(0, colonIdx), 10);
		if (!Number.isInteger(version) || version < 0) {
			throw new ClearanceError(
				`Invalid version in CLEARANCE_SECRETS: "${entry.slice(0, colonIdx)}". Version must be a non-negative integer.`,
			);
		}
		const value = entry.slice(colonIdx + 1).trim();
		if (!value) {
			throw new ClearanceError(
				`Empty secret value for version ${version} in CLEARANCE_SECRETS.`,
			);
		}
...
export function validateSecretsArray(
	...
	if (current.value.length < 32) {
		logger.warn(
			`[clearance] Warning: the current secret (version ${current.version}) should be at least 32 characters long for adequate security.`,
		);
	}
	const entropy = estimateEntropy(current.value);
	if (entropy < 120) {
		logger.warn(
			"[clearance] Warning: the current secret appears low-entropy. Use a randomly generated secret for production.",
		);
	}
}
