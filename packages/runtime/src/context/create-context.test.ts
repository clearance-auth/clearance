...
describe("base context creation", () => {
	const initBase = async (options: Partial<ClearanceOptions> = {}) => {
	...
	it("should infer BASE_URL from env", async () => {
		vi.stubEnv("CLEARANCE_URL", "http://localhost:5147");
	...
	describe("secret management", () => {
		...
		it("should use CLEARANCE_SECRET from env", async () => {
			vi.stubEnv(
				"CLEARANCE_SECRET",
				"clearance-secret-that-is-long-enough-for-validation",
			);
			const opts: ClearanceOptions = {};
			const adapter = await getAdapter(opts);
			const getDatabaseType = () => "memory";
	...
	describe("app name", () => {
		it("should use default app name", async () => {
			const res = await initBase({});
			expect(res.appName).toBe("Clearance");
		});
	...
	describe("trusted origins - environment variables", () => {
		it("should include origins from CLEARANCE_TRUSTED_ORIGINS env", async () => {
			vi.stubEnv(
				"CLEARANCE_TRUSTED_ORIGINS",
	...
	describe("secret validation", () => {
		...
		it("should throw error when default secret is set in production environment", async () => {
			...
			vi.doMock("@clearance/core/env", async () => {
				const actual = await vi.importActual("@clearance/core/env");
				return {
					...actual,
					isProduction: true,
					isTest: () => false,
			...
			const initBaseProduction = async (
				options: Partial<ClearanceOptions> = {},
			) => {
				const opts: ClearanceOptions = {
					baseURL: "http://localhost:3000",
					...options,
			...
			vi.doUnmock("@clearance/core/env");
			vi.resetModules();
