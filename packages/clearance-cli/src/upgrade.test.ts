...
describe("upgrade script lane", () => {
	it("ships the supported 0.1.2 to 0.2.0 transition hook", () => {
		const root = resolve(dirname(fileURLToPath(import.meta.url)), "../../..");
		const hook = join(root, "deploy/upgrades/steps/0.2.0/apply.sh");
		expect(existsSync(hook)).toBe(true);
		expect(readFileSync(hook, "utf8")).toContain('[[ "$FROM" == "0.1.2" && "$TO" == "0.2.0" ]]');
	});

	it("creates an offline plan in an isolated artifact directory", async () => {
		const dir = directory();
		const result = await planUpgrade({ target: "0.2.0", current: "0.1.2", dir });
		expect(result).toMatchObject({ schemaVersion: "v1", operation: "upgrade.plan", dryRun: false, plan: { currentVersion: "0.1.2", targetVersion: "0.2.0", status: "planned" } });
	...
	it("supports apply dry-run without confirmation and requires confirmation for mutation", async () => {
		const dir = directory();
		const planned = await planUpgrade({ target: "0.2.0", current: "0.1.2", dir });
	...
	it("rejects a symlinked artifact directory", async () => {
		...
		await expect(planUpgrade({ target: "0.2.0", current: "0.1.2", dir: link })).rejects.toMatchObject({ code: "UPGRADE_DIR_UNSAFE" });
	});
