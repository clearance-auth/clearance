...
function credentialOperationId(
	value: unknown,
	global: Readonly<DispatchInput<AccessCommandPath>["global"]>,
): string | undefined {
	if (global.dryRun) {
		if (value === undefined) return undefined;
		throw error(
			"SERVICE_ACCOUNT_CREDENTIAL_OPERATION_ID_DRY_RUN_INVALID",
			"--operation-id cannot be used with a credential dry run.",
			"Omit --operation-id for a preview, then provide it on the live command if you need retry recovery.",
		);
	}
	if (value === undefined) return randomUUID();
	if (typeof value === "string" && OPERATION_ID.test(value)) return value;
	throw error(
		"SERVICE_ACCOUNT_CREDENTIAL_OPERATION_ID_INVALID",
		"--operation-id must be a canonical UUID.",
		"Pass a UUID generated by your automation, or omit --operation-id to generate one.",
	);
}
	...
	switch (path) {
		...
		case SERVICE_ACCOUNT_OPERATIONS.credentialCreate.cliPath:
			return callManagementOperation(session, "service-accounts.credentials.create", body({
				...
				expiresAt: opts.expiresAt,
				dryRun: global.dryRun,
				operationId: credentialOperationId(opts.operationId, global),
			}) as
				| { organizationId: string; accountId: string; expiresAt?: string; dryRun: true }
				| { organizationId: string; accountId: string; expiresAt?: string; dryRun?: false; operationId: string }, managementCallOptions(global));
