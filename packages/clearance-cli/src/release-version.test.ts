...
describe("release version consistency", () => {
	it("accepts the current release and rejects drift in a release-bearing surface", () => {
		expect(execFileSync(process.execPath, [script, "0.1.1"], { encoding: "utf8" })).toContain("RELEASE_VERSION_OK");
		...
		writeFileSync(api, readFileSync(api, "utf8").replace('version: "0.1.1",', 'version: "9.9.9",'));
		expect(() => execFileSync(process.execPath, [script, "0.1.1"], {
			encoding: "utf8",
