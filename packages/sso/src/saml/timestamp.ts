...
export function validateSAMLTimestamp(
	...
	if (options.requireTimestamps && !conditions?.notOnOrAfter) {
		throw new APIError("BAD_REQUEST", {
			message: "SAML assertion missing required NotOnOrAfter expiry condition",
			details:
				"Strict timestamp validation requires a NotOnOrAfter condition that bounds assertion lifetime",
		});
	}
