...
import { parseSetCookieHeader, setCookieToHeader } from "@clearance/runtime/cookies";
import { bearer } from "@clearance/runtime/plugins";
...
describe("SSO Provider Config Parsing", () => {
	...
	it("returns parsed OIDC config and avoids [object Object] in response", async () => {
		...
		try {
			...
			expect(provider.oidcConfig?.clientSecret).toBeUndefined();
...
describe("SAML SSO - Timestamp Validation", () => {
	...
	describe("Missing timestamps behavior", () => {
		...
		it("should reject missing timestamps when requireTimestamps is true", () => {
			...
			).toThrow("SAML assertion missing required NotOnOrAfter expiry condition");
		});
		...
		it("should reject an empty NotOnOrAfter value in strict mode", () => {
			expect(() =>
				validateSAMLTimestamp(
					{ notOnOrAfter: "" },
		...
		it("should preserve NotBefore-only compatibility when strict mode is disabled", () => {
		...
		it("should reject NotBefore-only assertions in strict mode", () => {
			const now = new Date().toISOString();
			expect(() =>
				validateSAMLTimestamp(
					{ notBefore: now },
		...
		it("should accept a future NotOnOrAfter expiry in strict mode", () => {
			...
			const signUpRes = await auth.handler(
				new Request("http://localhost:8081/api/auth/sign-up/email", {
					...
					headers: {
						...
						Cookie: "clearance.session_token=fake-session",
					},
					body: JSON.stringify({
...
describe("SAML SSO - Single Assertion Validation", () => {
	...
	it("should normalize email to lowercase in SAML authentication to prevent duplicate creation", async () => {
		...
		const firstSession = await client.getSession({
			fetchOptions: {
				headers: {
					Cookie: `clearance.session_token=${firstSessionToken}`,
				},
			},
		});
		...
		const secondSession = await client.getSession({
			fetchOptions: {
				headers: {
					Cookie: `clearance.session_token=${secondSessionToken}`,
				},
			},
		});
...
describe("SAML SSO Hardening", () => {
	...
	/**
	 * Clearance specific: acsEndpoint provider lookup MUST fall back to the
	...
	/**
	 * Clearance specific: registration-time validation should catch config
