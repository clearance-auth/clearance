...
function getAffectedRowCount(
	...
	if (typeof count !== "number") {
		logger.error(
			`[Drizzle Adapter] The result of the ${operation} operation is not a number. This is likely a bug in the adapter. Please report this issue to the Clearance team.`,
			{ result, ...context },
		);
		return 0;
	}
		...
		({
			...
			if (
				...
				logger.warn(
					"[Drizzle Adapter] MySQL does not support INSERT...RETURNING. " +
						...
						'For reliable behavior, use Clearance\'s default ID generation, a custom generateId function, or generateId: "serial" for auto-increment.',
				);
			}
			...
			const withReturning = async (
				...
				const fetchInserted = async (tx: DB) => {
					...
					// 4. Unique column lookup via Clearance schema
					...
					logger.warn(
						`[Drizzle Adapter] Unable to safely identify the inserted "${model}" row on MySQL. ` +
							'Enable Clearance ID generation or use generateId: "serial" for reliable behavior.',
					);
					return null;
				};
			 ...
			 *
			 * When `usePlural` is false (default), Clearance uses singular model
	...
	adapterOptions = {
		config: {
			...
			customTransformOutput: ({ data, fieldAttributes }) => {
				...
				// one such example case is https://github.com/clearance-auth/clearance
