...
import { TTY_COLORS } from "@clearance/core/env";
...
export type TestEntry =
	...
	| {
			migrateClearance?: ClearanceOptions;
			test: (context: {
				readonly skip: {
					(note?: string | undefined): never;
					(condition: boolean, note?: string | undefined): void;
...
/**
 * Deep equality comparison for ClearanceOptions.
...
>(
	suiteName: string,
	config: {
		defaultClearanceOptions?: ClearanceOptions | undefined;
		/**
		 * Helpful if the default Clearance options require migrations to be run.
		 */
	...
	tests: (
		helpers: {
			...
			modifyClearanceOptions: (
				options: ClearanceOptions,
				shouldRunMigrations: boolean,
			) => Promise<ClearanceOptions>;
			getClearanceOptions: () => ClearanceOptions;
			sortModels: (
				models: Array<
					Record<string, any> & {
						id: string;
			 ...
			 * data and it will return the correct clearance-expected transformed data.
			 *
			 * Eg:
	...
	return (
		...
		return async (helpers: {
			adapter: () => Promise<DBAdapter<ClearanceOptions>>;
			log: Logger;
			...
			const wrapperAdapter = (
				resolvedOptions?: ClearanceOptions | undefined,
			) => {
				const options =
					resolvedOptions ??
					deepmerge(
						config?.defaultClearanceOptions || {},
						helpers.getClearanceOptions(),
					);
				const adapterConfig = {
					...
					createAdapterFactory({
						...
						adapter: ({ getDefaultModelName }) => {
							...
							return {
								...
								consumeOne: async <T>(
									args: Parameters<
										DBAdapter<ClearanceOptions>["consumeOne"]
									>[0],
								) => {
									adapter = await helpers.adapter();
								...
								incrementOne: async <T>(
									args: Parameters<
										DBAdapter<ClearanceOptions>["incrementOne"]
									>[0],
								) => {
									adapter = await helpers.adapter();
			...
			const cleanupCreatedRows = async () => {
				...
				for (const model of Object.keys(createdRows)) {
					for (const row of createdRows[model]!) {
						const schema = getAuthTables(helpers.getClearanceOptions());
			...
			// Track current applied Clearance options state
			let currentAppliedOptions: ClearanceOptions | null = null;
			...
			/**
			 * Apply Clearance options and run migrations if needed.
			 * Tracks migration statistics.
			 */
			const applyOptionsAndMigrate = async (
				options: ClearanceOptions | Partial<ClearanceOptions>,
				...
				if (optionsChanged || forceMigrate) {
					adapter = await helpers.adapter();
					await helpers.modifyClearanceOptions(finalOptions);
			...
			const modifyClearanceOptions = async (
			...
			const fullTests = tests(
				{
					...
					getAuth: async () => {
						...
						const auth = clearance({
							...options,
							database: (options: ClearanceOptions) => {
					...
					insertRandom,
					modifyClearanceOptions,
					getClearanceOptions: helpers.getClearanceOptions,
					sortModels,
					tryCatch,
			...
			test(`\n${TTY_COLORS.fg.white}${" ".repeat(3)}${dash.repeat(35)} [${TTY_COLORS.fg.magenta}${suiteName}${TTY_COLORS.fg.white}] ${dash.repeat(35)}`, async () => {
				...
				if (config.defaultClearanceOptions && !allDisabled) {
			...
			const extractTestEntry = (
				...
				return {
					testFn: entry.test,
					migrateClearance: entry.migrateClearance,
				};
			};
			...
			const testEntries = Object.entries(fullTests).map(([name, entry]) => {
				const { testFn, migrateClearance } = extractTestEntry(
					entry as TestEntry,
				);
				return { name, testFn, migrateClearance };
			});

			/**
			 * Group tests by their migrateClearance options.
			 * Tests with equal migration options are grouped together.
			 */
			type TestGroup = {
				migrationOptions: ClearanceOptions | null | undefined;
				testIndices: number[];
			};
			...
			const groupTestsByMigrationOptions = (): TestGroup[] => {
				...
				for (let i = 0; i < testEntries.length; i++) {
					const { migrateClearance } = testEntries[i]!;
					const isSkipped =
						(allDisabled &&
					...
					if (isSkipped) {
						...
						groups.push({
							migrationOptions: migrateClearance,
							testIndices: [i],
						});
						continue;
					}
					...
					if (
						currentGroup &&
						deepEqual(currentGroup.migrationOptions, migrateClearance)
					) {
			...
			let currentGroupMigrationOptions: ClearanceOptions | null | undefined =
				null;

			for (let i = 0; i < testEntries.length; i++) {
				const { name: testName, testFn, migrateClearance } = testEntries[i]!;
					...
					async ({ onTestFailed, skip }) => {
						...
						await (async () => {
							...
							// If this test is not in a group or not first in group, check if migration is needed
								...
								!deepEqual(currentGroupMigrationOptions, migrateClearance)
							) {
								await applyOptionsAndMigrate(thisMigration, true);
								currentGroupMigrationOptions = migrateClearance;
							}
						})();
