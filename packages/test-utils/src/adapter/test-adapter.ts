import type { Awaitable, ClearanceOptions } from "@clearance/core";
import type { DBAdapter } from "@clearance/core/db/adapter";
import { deepmerge, initGetModelName } from "@clearance/core/db/adapter";
import { TTY_COLORS } from "@clearance/core/env";
import { getAuthTables } from "@clearance/runtime/db";
...
export const testAdapter = async ({
	...
	) => Awaitable<(options: ClearanceOptions) => DBAdapter<ClearanceOptions>>;
	/**
	 * A function that will run the database migrations.
	 */
	runMigrations: (clearanceOptions: ClearanceOptions) => Promise<void> | void;
	/**
	 * Any potential clearance options overrides.
	 */
	overrideClearanceOptions?: (
	...
	const defaultBAOptions = {} satisfies ClearanceOptions;
	let clearanceOptions = (() => {
		return {
			...defaultBAOptions,
			...(overrideClearanceOptions?.(defaultBAOptions) || {}),
		} satisfies ClearanceOptions;
	})();

	let adapter: DBAdapter<ClearanceOptions> = (
		await getAdapter(clearanceOptions)
	)(clearanceOptions);
	...
	const refreshAdapter = async (clearanceOptions: ClearanceOptions) => {
		adapter = (await getAdapter(clearanceOptions))(clearanceOptions);
	};
	...
	const cleanup = async () => {
		...
		await refreshAdapter(clearanceOptions);
		const getAllModels = getAuthTables(clearanceOptions);
	...
	const migrate = async () => {
		...
		try {
			await runMigrations(clearanceOptions);
		} catch (error) {
			...
			describe(adapterDisplayName, async () => {
				...
				for (const testSuite of tests) {
					await testSuite({
						...
						modifyClearanceOptions: async (options) => {
							const newOptions = deepmerge(defaultBAOptions, options);
							clearanceOptions = deepmerge(
								newOptions,
								overrideClearanceOptions?.(newOptions) || {},
							);
							await refreshAdapter(clearanceOptions);
