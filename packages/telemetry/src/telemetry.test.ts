...
beforeEach(() => {
	vi.resetModules();
	process.env.CLEARANCE_TELEMETRY = "";
	process.env.CLEARANCE_TELEMETRY_DEBUG = "";
});
...
describe("telemetry", () => {
	...
	it("does not publish when disabled via env", async () => {
		process.env.CLEARANCE_TELEMETRY = "false";
		let event: TelemetryEvent | undefined;
		const track = vi.fn().mockImplementation(async (e) => {
			event = e;
		});
	...
	it("returns noop publisher when CLEARANCE_TELEMETRY_ENDPOINT is undefined", async () => {
		...
		vi.stubEnv("CLEARANCE_TELEMETRY_ENDPOINT", undefined);
