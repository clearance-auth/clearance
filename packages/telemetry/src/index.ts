...
import { ENV, getBooleanEnvVar, isTest, logger } from "@clearance/core/env";
...
export async function createTelemetry(
	...
	const debugEnabled =
		...
		getBooleanEnvVar("CLEARANCE_TELEMETRY_DEBUG", false);

	const telemetryEndpoint = ENV.CLEARANCE_TELEMETRY_ENDPOINT;
	...
	const isEnabled = async () => {
		const telemetryEnabled =
			options.telemetry?.enabled !== undefined
				...
				: false;
		const envEnabled = getBooleanEnvVar("CLEARANCE_TELEMETRY", false);
		return (
