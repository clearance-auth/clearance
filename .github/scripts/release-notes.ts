...
function packageToDir(name: string): string {
	...
	if (name === "clearance") return "packages/runtime";
	return `packages/${name.replace(/^@clearance\//, "")}`;
}
...
function formatReleaseBody(opts: FormatOptions): string {
	...
	if (minorMatch) {
		...
		lines.push(
			`**Blog post:** [Clearance ${minorMatch[1]}.${minorMatch[2]}](https://github.com/clearance-auth/clearance)`,
		);
		lines.push("");
	}
	...
	// Sort packages: clearance first, then by breaking count desc,
	// then by total entry count desc, then alphabetically
	const packageOrder = [...grouped.keys()].sort((a, b) => {
		if (a === "clearance") return -1;
		if (b === "clearance") return 1;
