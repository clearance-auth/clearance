...
- [ ] Version delivery fingerprint keys and prove rotation without losing queued jobs or weakening generation dedupe.
- [ ] Ship scoped delivery inspection, readiness, quotas, retry/cancel/replay controls, and exact CLI/API parity.
- [ ] Add signed webhook and SES transports, quota enforcement, observability, and deployment manifests.
...
| 2026-07-15 | Proved the integrated delivery slice through the canonical Postgres gate | Manager rerun: management 250/250, delivery 12/12, delivery-worker 14/14, and clearance-auth 22/22, all with machine-asserted zero skips; six affected packages typecheck and build; focused affected-runtime verification passed 162/163 with one pre-existing declared skip; `git diff --check` passed |
...
- Delivery uses Postgres-native immutable events, leased jobs, attempts, and worker heartbeats. Payloads and endpoint secrets are encrypted under a purpose-specific encryption keyring; fingerprint-key versioning and safe rotation remain an explicit acceptance blocker.
...
- Delivery semantics are at-least-once with stable event IDs, deterministic SMTP Message-ID values, bounded full-jitter retries, and dead-letter state. Provider acceptance is checkpointed before final completion when the lease remains owned; the unavoidable crash boundary after SMTP acceptance remains an at-least-once ambiguity. CLI retry/cancel/replay, exact-byte webhook HMAC signatures, and production webhook SSRF controls remain required.
