...
- [ ] Harden recovery and factor lifecycle with one-way recovery codes, step-up disable/regeneration, trusted-device revocation, and session revocation.
- [ ] Store session and refresh secrets by digest with rotation, reuse detection, family revocation, and short-lived asymmetric access-token integration.
- [ ] Add standards-complete WebAuthn/passkey registration, authentication, discovery, revocation, and recovery-safe policy behavior.
...
- Authentication security defaults are product-owned: TOTP and five-minute Ed25519 access tokens are enabled unless explicitly disabled, HIBP is enabled in strict/production mode, OAuth provider tokens use versioned authenticated envelopes, and public API types reflect disabled or dynamically configured plugins. Factor enrollment is one-row and idempotent, replacement requires the active factor and stages until proof of the new secret, TOTP timesteps are consumed atomically, and account attempt slots are reserved before attacker-controlled comparisons. Legacy signing keys retain their first retirement timestamp so repeated migration cannot extend grace. Recovery-code hashing, factor disable/regeneration step-up, trusted-device/session revocation, digest refresh families, passkeys, policy, KMS, runtime audit, and CLI control remain open gates.
