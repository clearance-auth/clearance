...
INSERT INTO clearance_management_snapshot(id, data) VALUES
  (1, '{"version":1,"releaseVersion":"0.1.3","meta":{"schemaVersion":1}}'::jsonb);
SQL
  ...
  # Upgrade flow
  PLAN_OUT="$(bash scripts/upgrade-plan.sh --target 0.2.0 --current 0.1.3 --dir "$UPG_DIR")"
  ...
  node -e '
    const fs=require("fs");
    const plan={
      planId:process.argv[1], immutable:true, currentVersion:"0.1.3",
  ...
  NO_HOOK_PLAN_OUT="$(bash scripts/upgrade-plan.sh --target 0.2.1 --current 0.1.3 --dir "$UPG_DIR")"
  ...
  # Exercise the real shipped 0.1.3 -> 0.2.0 hook from deploy/upgrades.
  bash scripts/upgrade-apply.sh --plan "$PLAN_ID" --dir "$UPG_DIR" --backup-dir "$BAK_DIR" \
    >/dev/null 2>"$SCRATCH/apply-success.err" \
    && ok "upgrade-apply executes the shipped 0.1.3 to 0.2.0 hook" \
    || bad "upgrade-apply failed with the shipped 0.1.3 to 0.2.0 hook"
  ...
  if [[ "$ACTIVE_OID_BEFORE" != "$ACTIVE_OID_AFTER" && "$RELEASE_ROLLED_BACK" == "0.1.3" && "$PRESERVED_POST_SAFETY" == "1" ]] \
  ...
  FAIL_PLAN_OUT="$(bash scripts/upgrade-plan.sh --target 0.2.0 --current 0.1.3 --dir "$UPG_DIR")"
