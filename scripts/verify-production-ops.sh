...
INSERT INTO clearance_management_snapshot(id, data) VALUES
  (1, '{"version":1,"releaseVersion":"0.1.4","meta":{"schemaVersion":1}}'::jsonb);
SQL
  ...
  node -e '
    const fs=require("fs");
    const plan={
      planId:process.argv[1], immutable:true, currentVersion:"0.1.4",
  ...
  if [[ "$ACTIVE_OID_BEFORE" != "$ACTIVE_OID_AFTER" && "$RELEASE_ROLLED_BACK" == "0.1.4" && "$PRESERVED_POST_SAFETY" == "1" ]] \
